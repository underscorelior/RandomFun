...
async def timeup(ctx,stt):
    ...
    embed = discord.Embed(
        title="Uptime",
        description=f"Current uptime: \n> {format_seconds(round((datetime.now() - stt).total_seconds()))}\n\nDiscord Timestamps: \n> <t:{upts}:F> (<t:{upts}:R>)",
        color=0x28acb8,
        timestamp=datetime.utcnow()
    )
    await ctx.send(embed=embed)

async def brr(ctx: commands.Context, eng):
    await ctx.send(
        f"English: {eng}\n"
        f'Braille: {(str(eng).lower()).replace("a", "⠁").replace("b", "⠃").replace("c", "⠉").replace("d", "⠙").replace("e", "⠑").replace("f", "⠋").replace("g", "⠛").replace("h", "⠓").replace("i", "⠊").replace("j", "⠚").replace("k", "⠅").replace("l", "⠅").replace("m", "⠍").replace("n", "⠝").replace("o", "⠕").replace("p", "⠏").replace("q", "⠟").replace("r", "⠗").replace("s", "⠎").replace("t", "⠞").replace("u", "⠥").replace("v", "⠧").replace("w", "⠺").replace("x", "⠭").replace("y", "⠽").replace("z", "⠵")}'
    )
...
class Random(commands.Cog):
    ...
    @cog_ext.cog_slash(
        name='httpcat',
        description='Find some httpcats!',
        options=[
            create_option(
                name="error",
                description="Get a cat with selected error code, or get a random one!",
                option_type=3,
                required=False
            )
        ]
    )
    async def httpcat(self, ctx: SlashContext, error: int = 404):
        await httpscat(ctx,error)

    @commands.command()
    async def httpcat(self, ctx, error: int = 404):
        await httpscat(ctx, error)
    ...
    async def uptime(self, ctx: SlashContext):
        await timeup(ctx,self.start_time)
