...
class Random(commands.Cog):
	...
	async def wikihowsh(self,ctx:SlashContext,query):
		...
		message = await ctx.send(await whs(ctx,query),components=components)
	...
	async def wikihow(self,ctx,wiki):
		...
		message = await ctx.send(content=await whs(ctx,wiki),components=delwh)
		try: 
