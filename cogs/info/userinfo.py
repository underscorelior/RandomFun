...
async def userinfoemb(ctx,user):
	...
	uinfemb = discord.Embed(title=f"{user.name}", description=f"💳 ID: {user.id}", color=clr)
	uinfemb.set_thumbnail(url=user.avatar_url)
	...
	if str(user.status).lower() == "online":
		...
		ustatem = "<:OFF:920502852315664405>"
	...
	uinfemb.timestamp = datetime.datetime.utcnow()
