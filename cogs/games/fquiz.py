...
class FlagQuiz(commands.Cog):
	...
	async def fquiz(self,ctx):
		...
		msem = discord.Embed(title=f'Which country does this flag belong to?',color=0x1860cc, timestamp = datetime.utcnow()).set_image(url=quizans["flags"]["png"]).set_footer(text=ctx.author,icon_url=ctx.author)
		...
		if int(ansch.custom_id) == ansloc:
			await message.edit(embed=discord.Embed(title='Win',description=f'Which country does this flag belong to? \nAnswer: `{quizans["name"]["common"]}`', color=0x3cb556, timestamp = datetime.utcnow()).set_thumbnail(url=quizans["flags"]["png"]).set_footer(text=ctx.author,icon_url=ctx.author),components=await winbtn(ansloc))
		else:
			qta = await losebtn(ctx,int(ansch.custom_id),ansloc)
			await message.edit(embed=discord.Embed(title='Lose',description=f'Which country does this flag belong to?: \nSelected Answer: `{btnans[0][qta[1]-1].label}` \nReal Answer: `{quizans["name"]["common"]}`',color=0xfa8e23, timestamp = datetime.utcnow()).set_thumbnail(url=quizans["flags"]["png"]).set_footer(text=ctx.author,icon_url=ctx.author),components=qta[0])
